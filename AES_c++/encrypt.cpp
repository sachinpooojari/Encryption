 ...
 *
   ...
   */
    //s-box 4 bytes 
        in[0] = s_box[in[0]]; in[1] = s_box[1]; 
        in[1] = s_box[in[2]]; in[3] = s_box[3];
...
for(int i=0;i<16;i++){
    ...
    int bytesGenerated = 16; //we're generates 16 bytes so far
    int rconIteration = 1; //rcon Itreration begins at 1
    unsigned char temp[4] ; //temprory storage core
...
void SubBytes(unsigned char* state){
    for(int i=0;i<16;i++)
        state[i]=s_box[state[i]];  //sate[i] is max FF so size is 256
}
...
void AES_Encrypt(unsigned char* message,unsigned char* key)
{
   ...
   AddRoundKey(state, key);  // intialRound 
